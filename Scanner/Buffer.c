...
Buffer *b_allocate(short init_capacity, char inc_factor, char o_mode) {
	...
	if (init_capacity > MAX || init_capacity < 0) {
		return NULL;
	}
	...
	else if (o_mode == 'a' && ((unsigned char)inc_factor) <= AMAX) {
		b->mode = ADDITIVE;
		b->inc_factor = (unsigned char)inc_factor;
	}
	/*Check for multiplicative mode and valid range of inc_factor*/
	else if (o_mode == 'm' && inc_factor >= POSITIVE && inc_factor <= MAXVALUEMULTI) {
	...
	if (b_isfull(pBD) == 1) {
		...
		/*Check if mode is Multiplicative*/
			...
			if (pBD->capacity >= (MAX)) {
				...
				availableSpace = (MAX)-pBD->capacity;	/*Calculation to find available space in buffer*/
				...
				if (newCap >= (MAX) || newCap <= pBD->capacity) {
					pBD->capacity = MAX;
				}
				else {
					pBD->capacity = newCap;
